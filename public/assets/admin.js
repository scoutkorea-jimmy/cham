/* ============================================================
   ...
   ============================================================ */
/* 이 파일은 **ES 모듈**이다(admin.html 의 <script type="module">).
   예전에는 IIFE 로 감싸 전역을 더럽히지 않았는데, 모듈은 그 자체가 자기 범위라
   껍데기가 필요 없다. 모듈은 언제나 엄격 모드이므로 'use strict' 도 뺐다.

   **왜 모듈로 바꿨나.** 이 파일 하나가 4천 줄이 넘어 무엇이 어디 있는지 찾기 어렵다.
   빌드 도구가 없는 저장소라 모듈이 유일하게 파일을 나눌 수 있는 수단이다.
   지금은 껍데기만 벗긴 상태이고, 화면 단위로 하나씩 떼어 낸다.
...
var KMS_DEFAULTS = {
  standard: [
    ...
    '- robots.txt / sitemap.xml 은 functions/ 가 요청 호스트로 만든다 — 도메인이 바뀌어도 손댈 것이 없다',
    '- llms.txt 는 정적 파일이고 링크가 상대경로다. 공개 페이지가 늘면 여기도 늘린다',
