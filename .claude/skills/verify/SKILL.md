...
8. **배포 직후 검증은 옛 코드를 볼 수 있다.** 배포 성공 뒤에도 잠깐은 이전 자산이 온다 —
   ...
   ```bash
   npx wrangler pages deployment list --project-name cham | head -6   # Source 열이 커밋 해시
   ```

   응답으로 확인하려면 **새 코드에만 있는 신호**를 골라야 한다.
   *옛 코드에서도 참인 신호는 배포 여부를 가르지 못한다.*
   실제로 빈도 제한을 배포하고 `POST /api/submit` 에 잘못된 본문을 던져 400 이 오는 것을
   신호로 삼았는데, **그 400 은 바꾸기 전에도 나오던 응답**이었다 — 옛 코드를 시험하며
   "제한이 안 걸린다"고 코드를 뒤졌다. 골랐어야 할 신호는 429 나 `code: 'throttled'` 였다.
